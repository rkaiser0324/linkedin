...
class LinkedInStrategy extends OpauthStrategy
{
    ...
    public function oauth2callback()
    {
        if (array_key_exists('code', $_GET) && !empty($_GET['code'])) {
            ...
            if (!empty($results) && !empty($results->access_token)) {
                ...
                $this->mapProfile($profile, "firstName.localized." . key($profile['firstName']['localized']), 'info.first_name');
                $this->mapProfile($profile, "lastName.localized." . key($profile['lastName']['localized']), 'info.last_name');
                $this->mapProfile($profile, 'profilePicture.displayImage~.elements.0.identifiers.0.identifier', 'info.image');
    ...
    /**
     * Queries LinkedIn API v2 for user profile and email address.
     *
    ...
    private function getProfile($access_token)
    {
        ...
        $response = $this->serverGet('https://api.linkedin.com/v2/me', array('oauth2_access_token' => $access_token, 'projection' => $fields));

        if (!empty($response)) {
            $profile = json_decode($response, true);

            $response = $this->serverGet('https://api.linkedin.com/v2/emailAddress', [
            ...
            if (!empty($response)) {
                ...
                $profile['emailAddress'] = $response_arr['elements'][0]['handle~']['emailAddress'];
            }
